...
export default function Feedback() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!error) { setName(''); setMessage(''); setRating(5) } else { alert(error.message) }
  }
    ...
    <div className="space-y-6">
      <form onSubmit={onSubmit} className="grid gap-4 p-4 rounded-xl border bg-white/70">
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <button type="submit" className="ml-auto px-4 py-2 rounded-lg bg-green-700 text-white hover:scale-105 transition">
            Submit
          </button>
        </div>
      </form>
